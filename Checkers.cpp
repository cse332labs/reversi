        ...
        :abstractGame(), isBlacksTurn_(true), currentMoveType_(EMPTY), Bcount_(0), Rcount_(0)
{
        ...
        //debug pieces
        board_[Point(0,0)] = black;
        black.crown();
        board_[Point(0,2)] = black;
        board_[Point(1,3)] = red;
...
bool Checkers :: done()
{
        bool done = false, opponentHasMove = false;

        if(Rcount_ == 0 || Bcount_ == 0)
        {
                done = true;
        }
	return done;
...
endCondition Checkers :: play()
{
        ...
        while(state_ != FINISHED)
        {
                ...
                if(done())
                {
                        string winner = playerR_;
                        if(isBlacksTurn_)
                        {
                                winner = playerB_;
                        }
                        cout << "Congratulations! " << winner << " is the winner! Thank you for playing!" << endl;
                        this->noSave();
                }
